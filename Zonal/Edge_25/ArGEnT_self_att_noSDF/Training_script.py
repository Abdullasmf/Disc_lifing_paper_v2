...
Perc_training_data: float = 0.25  # fraction of training data to keep and rest to discard (for ablation studies)
...
def main(preset_name: str = "S0", batch=8) -> None:
    ...
    # Randomly subsample training data if requested (e.g., for ablation studies)
    train_tensors = train_tensors[:int(len(train_tensors) * Perc_training_data)]
    print(f"Using {len(train_tensors)} training geometries ({Perc_training_data*100:.1f}% of total).")
