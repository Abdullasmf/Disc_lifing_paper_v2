...
EXTRA_FEAT_COLS: List[int] = [3, 4, 5, 6, 7]
HEAD_FEAT_COLS: List[int] = [3, 4, 5, 6, 7]
...
class GeomLifeDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        return {
            ...
            "head_feats": head_feats_n,  # [N,Fh] normalized query features
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                with torch.cuda.amp.autocast(enabled=(device.type == "cuda" and use_amp)):
                    ...
                    pred = model(geom_xyz.unsqueeze(0), pts.unsqueeze(0), gf_val, head_feats.unsqueeze(0))   # [1,N,2]
...
def main(preset_name: str = "S0", batch=8) -> None:
    ...
    if width0 < 4:
        raise RuntimeError(
            f"Dataset width {width0} is too narrow for Edge_arc_feat ablation "
            f"(need at least 10 columns). Wrong H5 file?"
        )
    ...
    fp_cfg = _cfg.get("fp", None)   # e.g. {"fp2": [256, 128], "fp1": [128, 64]}
    ...
    if resume_checkpoint is not None:
        ...
        if resume_checkpoint.get("model_family") != "PointNetMLPJoint_FP_headfeat":
        ...
        if resume_checkpoint.get("headfeatcols", resume_checkpoint.get("head_feat_cols")) != HEAD_FEAT_COLS:
            raise RuntimeError("Checkpoint headfeatcols do not match this FP head-feature ablation.")
        ...
        if resume_checkpoint.get("extra_feat_stats") is not None:
            ...
            if resume_checkpoint.get("extra_feat_cols") is not None:
                ...
                if _ckpt_efc != EXTRA_FEAT_COLS:
                    raise RuntimeError(
                        f"Checkpoint extra_feat_cols {_ckpt_efc} != ablation EXTRA_FEAT_COLS {EXTRA_FEAT_COLS}. "
