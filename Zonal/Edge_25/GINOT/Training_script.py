...
train_data_percent: float = 0.25  # fraction of training data to take for ablation (0.0-1.0)
...
def main(
    ...
    if dry_run:
        EXTRA_FEAT_COLS = []
        GINOT_A(cfg=ginot_cfg, out_dim=NUM_TARGETS, in_channels=len(EXTRA_FEAT_COLS))
    ...
    save_path = save_path_override if save_path_override is not None else (save_dir / f"{base_name}_{arch_hash}_{train_data_percent:.2f}.pt")
    ...
    #remove random data from training dataset depending on train_data_percent
    train_tensors = train_tensors[: int(len(train_tensors) * train_data_percent)]
    print(f"Training on {len(train_tensors)} geometries ({train_data_percent*100:.0f}% of total), validating on {len(val_tensors)} geometries.")
