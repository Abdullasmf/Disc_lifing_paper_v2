...
def life_weights(target_z: torch.Tensor) -> torch.Tensor:
    """Per-node loss weight ramping 1.0 -> 5.0 as standardized log-life falls from +2 to -2 sigma.

    Anchored to the training-set target statistics already used for standardization,
    so weights are identical across batches, epochs and train/val.
    """
    t = ((2.0 - target_z[..., -1]) / 4.0).clamp(0.0, 1.0)
    return (1.0 + 4.0 * t).unsqueeze(-1)
...
def weighted_loss(
    ...
    """Mean of per-channel node-averaged squared error; only the LogLife channel is life-weighted."""
    diff2 = (pred_z - target_z) ** 2
    m = (
        torch.ones_like(diff2[..., :1])
        if node_mask is None
        else node_mask.to(diff2.dtype)
    )
    w_life = m * life_weights(target_z).to(diff2.dtype)
    last = diff2.shape[-1] - 1
    terms: List[torch.Tensor] = []
    for c in range(diff2.shape[-1]):
        wc = w_life if c == last else m
        terms.append(
            (diff2[..., c : c + 1] * wc).sum() / wc.sum().clamp(min=1e-8)
        )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for batch in train_loader:
            ...
            with torch.cuda.amp.autocast(enabled=(device.type == "cuda" and use_amp)):
                ...
                loss = weighted_loss(pred_z, target_z, node_mask)
