...
def main(preset_name: str = "S0", batch=8) -> None:
    ...
    # # [fix_ablation_extra_feat_cols] patched: hardcoded for Edge_arc_feat ablation
    EXTRA_FEAT_COLS = [3, 4, 5, 6, 7]
    HEAD_FEAT_COLS = [3, 4, 5, 6, 7]  # same as EXTRA_FEAT_COLS for Edge_arc_feat
    if width0 < 10:
        raise RuntimeError(
            f"Dataset width {width0} is too narrow for Edge_arc_feat ablation "
            f"(need at least 10 columns). Wrong H5 file?"
        )
    print(f"EXTRA_FEAT_COLS=HEAD_FEAT_COLS=[3, 4, 5, 6, 7] (5 feature(s)) for Edge_arc_feat ablation")
    ...
    if resume_checkpoint is not None:
        ...
        if resume_checkpoint.get("extra_feat_stats") is not None:
            ...
            if resume_checkpoint.get("extra_feat_cols") is not None:
                ...
                if _ckpt_efc != [3, 4, 5, 6, 7]:
                    raise RuntimeError(
                        f"Checkpoint extra_feat_cols {_ckpt_efc} != ablation EXTRA_FEAT_COLS [3, 4, 5, 6, 7]. "
            ...
            if resume_checkpoint.get("head_feat_cols") is not None:
                ...
                if _ckpt_hfc != [3, 4, 5, 6, 7]:
                    raise RuntimeError(
                        f"Checkpoint head_feat_cols {_ckpt_hfc} != ablation HEAD_FEAT_COLS [3, 4, 5, 6, 7]. "
                        "Delete the stale checkpoint and retrain."
                    )
