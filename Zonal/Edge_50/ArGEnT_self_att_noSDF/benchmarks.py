...
class ArGEnTDeepONet(nn.Module):
    ...
    layers + residual output MLP) and projects the result to target channels.
    ...
    def __init__(
        ...
        # Final projection: hidden_dim → target channels
        self.trunk_proj = nn.Linear(hidden_dim, self.out_channels)
    ...
    def forward(
        ...
        out = self.trunk_proj(q.reshape(-1, self.hidden_dim)).view(B, Q_len, self.out_channels) + self.bias
