...
class PointNetMLPJoint_FP(nn.Module):
    def __init__(self, latent_dim=128, mlp_hidden=None, out_dim=1,
                 encoder_cfg=None, in_channels=0, headfeatdim=0):
        super().__init__()
        ...
        self.headfeatdim = int(headfeatdim)
        if self.headfeatdim < 0:
            raise ValueError("headfeatdim must be nonnegative")

        # Head input retains local FP features and adds query-aligned features.
        head_in_dim = fp1_out + self.latent_dim + q_in_dim + self.headfeatdim
    ...
    def forward(self, geom_xyz: torch.Tensor, query_points: torch.Tensor,
                geom_feats: Optional[torch.Tensor] = None,
                headfeats: Optional[torch.Tensor] = None) -> torch.Tensor:
        if geom_xyz.ndim != 3 or geom_xyz.shape[-1] != 2:
            raise RuntimeError(f"geom_xyz must have shape [B,N,2], got {tuple(geom_xyz.shape)}")
        if query_points.ndim != 3 or query_points.shape[-1] != 2:
            raise RuntimeError(f"query_points must have shape [B,Q,2], got {tuple(query_points.shape)}")
        if geom_xyz.shape[0] != query_points.shape[0]:
            raise RuntimeError("geom_xyz and query_points must have the same batch size")
        if geom_feats is not None and (geom_feats.ndim != 3 or geom_feats.shape[:2] != geom_xyz.shape[:2]):
            raise RuntimeError(f"geom_feats must have shape [B,N,F] aligned to geom_xyz, got {None if geom_feats is None else tuple(geom_feats.shape)}")
        if self.headfeatdim > 0:
            if headfeats is None or headfeats.numel() == 0:
                raise RuntimeError(f"headfeats are required when headfeatdim={self.headfeatdim}")
            if headfeats.ndim != 3 or headfeats.shape[:2] != query_points.shape[:2] or headfeats.shape[-1] != self.headfeatdim:
                raise RuntimeError(f"headfeats must have shape [B,Q,{self.headfeatdim}], got {tuple(headfeats.shape)}")
        elif headfeats is not None and headfeats.numel() > 0:
            raise RuntimeError("headfeats must be omitted or empty when headfeatdim=0")
        ...
        # 8. Head: [local FP feature | z | Fourier(query_xy) | head features]
        q_enc = self.head_posenc(query_points) if self.head_posenc is not None else query_points
        z_exp = z.unsqueeze(1).expand(-1, Q, -1)
        head_parts = [node_feats, z_exp, q_enc]
        if self.headfeatdim > 0:
            head_parts.append(headfeats)
        x = torch.cat(head_parts, dim=-1)        # [B, Q, head_in_dim]
        x = x.reshape(B * Q, -1)
...
def build_fp_model_from_arch(arch: Dict[str, Any]) -> PointNetMLPJoint_FP:
    ...
    head_hidden = arch.get("head_hidden", [256, 256, 128])
    out_dim = int(arch.get("out_dim", 1))
    headfeatdim = int(arch.get("headfeatdim", 0))
    latent_dim = int(encoder_cfg.get("latent_dim", 128)) if encoder_cfg else 128
